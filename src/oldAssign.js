...
import { createRiderRideeInfoMap, mapDayTypeUserArray} from "./service/responseHandlers";

class oldAssign extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            page: 0,
        ...
        this.rideeInfoMap = null;
        this.riderInfoMap = null;
    ...
    componentDidMount() {
        userService.getRiders().then(riders => {
            ...
            this.riderInfoMap = createRiderRideeInfoMap(riders.data);
        });
        userService.getRidees().then(ridees => {
            ...
            this.rideeInfoMap = createRiderRideeInfoMap(ridees.data);
        });
        userService.getQueue(helper.getDate(new Date())).then(res => {
            if (res.data) {
                ...
                userService.getRideAvailability({queueID: queueID}).then(res => {
                    if (res.data) {
                        this.setState({
                ...
                userService.getRideNeeded({queueID: queueID}).then(res => {
                    if (res.data) {
                        this.setState({
                            ridesNeeded: res.data
                        })
                    }
                });
            }
    ...
    addRideNeededRow(row) {
        const ridee = this.rideeInfoMap.get(row.ridee);
        const newRow = {
            key: this.state.rowIndex,
            day: row.day,
            ridee: ridee.NAME,
            address: ridee.ADDRESS,
            tel: ridee.PHONE_NUMBER,
            rider: '',
            leader: '',
            type: row.type,
            date: helper.findDateOfDay(this.state.queue.START_DATE)
        };
        const assignRows = this.state.assignRows.concat(newRow);
        this.setState({
            assignRows: assignRows,
